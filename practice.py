...
def halvesies(numbers):
    ...
    the half::
        ...
        [0.5, 2.5]
    """
    nums_div_by_two = [float(num)/2 for num in numbers]
    return nums_div_by_two
...
def join_strings(words):
    ...
    concat_strings = ''
    for word in words:
        concat_strings = concat_strings + word
...
def average(numbers):
    ...
    if numbers == []:
        ...
        for num in numbers:
            total_sum = total_sum + num
        avg = float(total_sum)/len(numbers)
        return avg
...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'berry', 'cherry']
    """
    reversed_list = [items[-(pos+1)] for pos in range(len(items))]
...
#     first_item = items[0]
#     items[0] = items[end]
#     items[end] = first_item
    
#     second_item = items[1]
#     items[1] = items[end-1]
#     items[end-1] = second_item
...
def duplicates(items):
    ...
    #         duplicate_items.append(items[pos])
    return set(duplicate_items)
